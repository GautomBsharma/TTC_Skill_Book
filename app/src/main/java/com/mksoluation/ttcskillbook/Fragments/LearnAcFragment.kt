...
import com.mksoluation.ttcskillbook.databinding.FragmentLearnAcBinding

class LearnAcFragment : Fragment() {

    private lateinit var binding: FragmentLearnAcBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentLearnAcBinding.inflate(layoutInflater)
        auth = FirebaseAuth.getInstance()
        val uid = auth.currentUser?.uid
        learnList = ArrayList()
        binding.learnRecycleAc.layoutManager = LinearLayoutManager(requireContext())
        adapter = learnList?.let { LearnAdapter(requireContext(), it) }!!
        binding.learnRecycleAc.adapter = adapter
        binding.learnRecycleAc.setHasFixedSize(true)
        ...
        binding.addLearAc.setOnClickListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    for (snap in snapshot.children){
                        ...
                        if (data != null) {
                            if (uid  == data.AdminId){
                                binding.addLearAc.visibility = View.VISIBLE
                            }
    ...
    private fun gettool() {
        val reff = FirebaseDatabase.getInstance().reference.child("Learn").child("Refrigeration//AC")

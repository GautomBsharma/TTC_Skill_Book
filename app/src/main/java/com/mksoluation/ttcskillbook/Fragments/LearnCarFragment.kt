...
import com.mksoluation.ttcskillbook.databinding.FragmentLearnCarBinding


class LearnCarFragment : Fragment() {
    private lateinit var binding: FragmentLearnCarBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentLearnCarBinding.inflate(layoutInflater)
        auth = FirebaseAuth.getInstance()
        val uid = auth.currentUser?.uid
        learnList = ArrayList()
        binding.recyclerCarLearn.layoutManager = LinearLayoutManager(requireContext())
        adapter = learnList?.let { LearnAdapter(requireContext(), it) }!!
        binding.recyclerCarLearn.adapter = adapter
        binding.recyclerCarLearn.setHasFixedSize(true)
        ...
        binding.addCarLearn.setOnClickListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    for (snap in snapshot.children){
                        ...
                        if (data != null) {
                            if (uid  == data.AdminId){
                                binding.addCarLearn.visibility = View.VISIBLE
                            }
    ...
    private fun gettool() {
        val reff = FirebaseDatabase.getInstance().reference.child("Learn").child("Driving")

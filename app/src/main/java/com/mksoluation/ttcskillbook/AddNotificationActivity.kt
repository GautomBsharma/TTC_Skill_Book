...
import com.mksoluation.ttcskillbook.databinding.ActivityAddNotificationBinding
import java.util.*
...
class AddNotificationActivity : AppCompatActivity() {
    ...
    private fun storeData2() {
        ...
        updateMap["NotificationImageUrl"] = ""
        updateMap["NotificationId"] = postId.toString()
        updateMap["description"] = binding.description.text.toString()
        updateMap["title"] = binding.tvTitle.text.toString()
    ...
    private fun uploadImages(uri: Uri) {
        ...
        val storageRef = FirebaseStorage.getInstance().reference.child("Notification/$fileName")
    ...
    private fun storeData(imageUrl: String) {
        ...
        val postId= dbRef.push().key
        val updateMap = HashMap<String,Any>()
        updateMap["NotificationImageUrl"] = imageUrl
        updateMap["NotificationId"] = postId.toString()
        updateMap["description"] = binding.description.text.toString()
        updateMap["title"] = binding.tvTitle.text.toString()
        if (postId != null) {
